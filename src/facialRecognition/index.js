...
export default async function (inputImage) {
     ...
     const labels = fs.readdirSync(path.join(__dirname, 'img/userFaces'));
     // getting array with all face user images knowed
     console.log('ya leyo los label')
     const labeledFaceDescriptors = await Promise.all(
          labels.map(async label => {
               const urlImg = path.join(__dirname, `img/userFaces/${label}`);
